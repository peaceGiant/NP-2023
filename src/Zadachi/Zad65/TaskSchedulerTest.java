...
class Schedulers {
    ...
    public static <T extends Task> TaskScheduler<T> getOrdered() {
        ...
        return new TaskScheduler<T>() {
            ...
            public List<T> schedule(T[] array) {
                return Arrays.stream(array).sorted(Comparator.comparing(Task::getOrder)).collect(Collectors.toList());
            }
        };
    ...
    public static <T extends Task> TaskScheduler<T> getFiltered(int order) {
        ...
        return array -> Arrays.stream(array)
                .filter(task -> task.getOrder() <= order).collect(Collectors.toList());
